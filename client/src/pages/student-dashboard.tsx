...
const RecentTestCard = ({ attempt }: { attempt: TestAttempt }) => {
  ...
  const isPassed = parseFloat(attempt.percentage) >= (attempt.test.passingMarks / attempt.test.totalMarks) * 100;
    ...
    <Card className="overflow-hidden border hover:shadow-md transition-shadow">
      ...
      <CardContent className="pb-3">
        <div className="flex justify-between items-center mb-2">
          <span className="text-sm font-medium">Score: {attempt.score}/{attempt.totalMarks}</span>
...
export default function StudentDashboard() {
  ...
  const { data: testAttempts, isLoading: isAttemptsLoading } = useQuery<TestAttempt[]>({
  ...
  const averageScore = testAttempts && testAttempts.length > 0
    ? (testAttempts.reduce((sum, attempt) => sum + parseFloat(attempt.percentage), 0) / testAttempts.length).toFixed(1)
    : '0';
  
  const passedTests = testAttempts?.filter(attempt => 
    parseFloat(attempt.percentage) >= (attempt.test.passingMarks / attempt.test.totalMarks) * 100
  ).length || 0;
  ...
  return (
    <div className="container mx-auto py-8 px-4">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
        ...
        <StatCard 
          title="Last Activity" 
          value={recentAttempts.length > 0 ? new Date(recentAttempts[0].endTime).toLocaleDateString() : 'N/A'} 
          ...
          <div className="bg-white rounded-lg border p-6">
            ...
            {testAttempts && testAttempts.length > 0 ? (
                        ...
                        .map(attempt => {
                          ...
                          return (
                            <div 
                              ...
                              onClick={() => navigate(`/test-report/${attempt.id}`)}
                            >
                              <div className="col-span-4 font-medium text-gray-800">{attempt.test.title}</div>
                              <div className="col-span-2 text-center">{attempt.score}/{attempt.totalMarks}</div>
                              <div className="col-span-2 text-center">{attempt.percentage}%</div>
              ...
              <div className="text-center py-12 bg-gray-50 rounded-lg">
                ...
                <p className="text-sm text-gray-500 mb-4">Complete some tests to see your performance analysis.</p>
                <Button onClick={() => navigate('/test-series')}>
                  Take a Test
                </Button>
              </div>
          ...
          <div className="bg-white rounded-lg border p-6">
            ...
            {testAttempts && testAttempts.length > 0 ? (
                  ...
                  .map(attempt => {
                    ...
                    return (
                      <div 
                        ...
                        onClick={() => navigate(`/test-report/${attempt.id}`)}
                      >
                        <div className={`h-10 w-10 rounded-full flex items-center justify-center ${isPassed ? 'bg-green-100' : 'bg-red-100'}`}>
                          {isPassed ? (
              ...
              <div className="text-center py-12 bg-gray-50 rounded-lg">
                ...
                <p className="text-sm text-gray-500 mb-4">Start taking tests to record your activity.</p>
                <Button onClick={() => navigate('/test-series')}>
                  Take a Test
                </Button>
              </div>
